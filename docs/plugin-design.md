...
- `includes/Frontend/Shortcodes.php`: شورتکدهای `[ms_booking_form]` و `[ms_provider_directory]` برای نمایش کارت پزشکان و فرم رزرو ساده روی صفحات سایت به‌همراه assetهای سبک در `assets/js|css/`؛ فرم رزرو در زمان انتخاب پزشک/تاریخ با `GET /ms/v1/availability` بازه‌های پر را نمایش می‌دهد تا کاربر تداخل را پیش از ارسال رزرو ببیند.
